#!/usr/bin/env python3
"""
step_mdRQA_wrists_velocity.py - Multidimensional Recurrence Quantification Analysis
of the wrists, using velocity profiles for the BalanceCorpus motion-tracking data.

This script embeds the multidimensional velocity (gradient of position with respect
to time) of the wrists at each frame and computes recurrence in that joint
velocity space.
...
def process_file(csv_path, window_sec=20.0, step_sec=1.0):
    """Run wrist mdRQA on a single time-series CSV using velocity; return the result dict or None."""
    ...
    print(f" Processing {len(velocity_embedding)} frames in {velocity_embedding.shape[1]}-D velocity space")

    # Proceed with RQA using velocity
    rec_matrix_full, threshold, rec_rate = calculate_recurrence_matrix(velocity_embedding)

    emb_vis, time_vis, rec_matrix_vis = downsample_for_visualization(
        velocity_embedding, time_clean, rec_matrix_full
    )
    ...
    result = {
        'analysis': 'mdRQA_velocity',
        'dimensions': [f"V_{col}" for col in wrist_cols],
        ...
        'visualization': {
            ...
            'data': {f"V_{col}": emb_vis[:, i].tolist() for i, col in enumerate(wrist_cols)},
            'matrix_size': len(time_vis),
            'sparse_matrix': sparse_matrix,
        },
        'full_data': {
            'n_points': len(velocity_embedding),
            'time_range': [float(time_clean[0]), float(time_clean[-1])],
        },
    }
    return result
...
def main():
    parser = argparse.ArgumentParser(description='Multidimensional wrist velocity RQA for the motion-tracking corpus')
    parser.add_argument('--input-dir', default=str(MOTION_DIR / 'Output_TimeSeries'),
                        help='Directory of time-series CSVs')
    parser.add_argument('--output-dir', default=str(MOTION_DIR / 'Output_mdRQA_wrists_velocity'),
